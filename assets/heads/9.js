...
root.style.setProperty('--color-primary', '#00000');
root.style.setProperty('--second-color', '#000000');
root.style.setProperty('--home-menu-color', '#00000');
head_color = 0X000;
...
p.draw = function() {
  ...
  p.x += (p.noise(p.frameCount * 0.01) - 0.5) * 90;
  p.y += (p.noise(p.frameCount * 0.02) - 0.5) * 90;

  if (p.x > p.width - 100) {
    p.px = p.x = 100;
  }
  if (p.x < 100) {
    p.px = p.x = p.width - 100;
  }
  if (p.y > p.height - 100) {
    p.py = p.y =  100;
  }
  if (p.y < 100) {
   p.py = p.y = p.height - 100;
  }

  p.ellipse(p.x, p.y, 150, 150);
